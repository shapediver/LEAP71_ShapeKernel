        ...
        public class ControlPointSurface
        {
                ...
                EEnds               eEndsV      = EEnds.OPEN)
            {
                ...
                if (m_eEndsU == EEnds.CLOSED)
                {
                    ...
                    if ((m_aControlGrid[0][0] - m_aControlGrid[m_aControlGrid.Count - 1][0]).Length() < m_fError)
                    {
                ...
                {
                    //check that first and last points are not the same!
                    if ((m_aControlGrid[0][0] - m_aControlGrid[0][m_aControlGrid[0].Count - 1]).Length() < m_fError)
                    {
            ...
            public float fBaseFunc(List<float> aKnot, float fLengthRatio, int nControlPoint, int nDegree)
            {
                ...
                if (nDegree == 0)
                {
                    ...
                    if (
                        ...
                        ((MathF.Abs(fLengthRatio - aKnot[nControlPoint + 1]) < m_fError) && (MathF.Abs(fLengthRatio - aKnot[aKnot.Count - 1]) < m_fError))
                        )
                    {
                        fValue = 1f;
                    }
